...
namespace lostPropertyMngt.DAL
{
	...
	/// 数据访问类:adminInfoDAL
	/// </summary>
	public partial class adminInfoDAL
	{
		public adminInfoDAL()
		{}
		#region  BasicMethod
		...
		public int Add(lostPropertyMngt.Model.adminInfo model)
		{
			...
			SqlParameter[] parameters = {
					new SqlParameter("@adminName", SqlDbType.NVarChar,15),
					new SqlParameter("@adminPwd", SqlDbType.VarChar,16)};
		...
		public lostPropertyMngt.Model.adminInfo GetModel(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,adminName,adminPwd from adminInfo ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public lostPropertyMngt.Model.adminInfo DataRowToModel(DataRow row)
		{
			lostPropertyMngt.Model.adminInfo model=new lostPropertyMngt.Model.adminInfo();
			if (row != null)
			{
